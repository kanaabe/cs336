...
<h2>Admin Page</h2>
	...
	<form name="input" method="post" action="logout.jsp">
		...
		<br>To upload a txt file of student registrations:<br>
		<form name="input" method="post" action="readstudentfile.jsp">
		Upload File: <input type="text" name="filename"><input type="submit" value="Scan Text File" />
		</form>
		
		<br>To upload a txt file of instructor registrations:<br>
		<form name="input" method="post" action="readinstructorfile.jsp">
		Upload File: <input type="text" name="filename"><input type="submit" value="Scan Text File" />
		</form>
		...
		<%
			try {				
				...
				//Courses
				rs = stmt.executeQuery("SELECT title,cid	 from course");
				
				out.println("<br>Current Courses");
				out.println("<table border=1 width=400>");
				out.println("<tr><td>    Course Name"  + "</td><td>    CID"  + "</td></tr>");
				while (rs.next()) {
					String title = rs.getString(1);
					int cid = rs.getInt(2);
					out.println("<tr><td>" + title + "</td><td>" + cid + "</td></tr>");
				} 
				out.println("</table>");
